...
public class UserCenterFollowingFragment extends UserBaseFragment<UserCenterUserListRecyclerAdapter> {

    private boolean mIsSelectAtUser;

    public static UserCenterFollowingFragment newInstance(User user) {
        return newInstance(user, false);
    }

    public static UserCenterFollowingFragment newInstance(User user, boolean isSelectAtUser) {
        Bundle args = new Bundle();
        args.putParcelable(EXTRA_USER, user);
        args.putBoolean("isSelectAtUser", isSelectAtUser);
        UserCenterFollowingFragment fragment = new UserCenterFollowingFragment();
    ...
    public void sendRequest() {
        new UserRequest().getFollowing(mUser.login, mPage.current(), Page.LIMIT, new Callback<FollowingArrayResponse>() {
            @Override
            public void onResponse(Response<FollowingArrayResponse> response, Retrofit retrofit) {
                stopRefresh(mSwipeRefreshLayout);
                if (response.isSuccess()) {
                    List<User> items = response.body().following;
